...
class BookingServiceStack(Stack):
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize Booking Service Stack
        ...
        service_sg = ec2.SecurityGroup(
            ...
            vpc=vpc,
            description='Security group for Booking Consumer ECS service',
        ...
        consumer_repo = ecr.Repository.from_repository_name(
            self, 'BookingServiceRepo', repository_name='booking-service'
        )
        ...
        task_def = ecs.FargateTaskDefinition(
            self,
            'TaskDef',
            memory_limit_mib=config['services']['booking']['task_memory'],
            cpu=config['services']['booking']['task_cpu'],
        ...
        task_def.add_container(
            ...
            environment={
                ...
                # Kvrocks (EC2 instance) - Used for seat availability cache
                ...
                'ACCESS_TOKEN_EXPIRE_MINUTES': str(
                    config.get('jwt', {}).get('access_token_expire_minutes', 30)
                ),
                'REFRESH_TOKEN_EXPIRE_DAYS': str(
                    config.get('jwt', {}).get('refresh_token_expire_days', 7)
                ),
                ...
                'OTEL_SERVICE_NAME': 'booking-consumer',
        ...
        adot = task_def.add_container(
            ...
            logging=ecs.LogDriver.aws_logs(
                stream_prefix='adot-booking-consumer', log_retention=logs.RetentionDays.ONE_WEEK
            ),
            environment={
...
""",
        ...
        service_name = f'ticketing-{env_name}-booking-service'
        ...
        service = ecs.FargateService(
            ...
            desired_count=config['services']['booking']['min_tasks'],
        ...
        scaling = service.auto_scale_task_count(
            min_capacity=config['services']['booking']['min_tasks'],
            max_capacity=config['services']['booking']['max_tasks'],
        )
        ...
        CfnOutput(
            ...
            description='Booking consumer service name',
        )
