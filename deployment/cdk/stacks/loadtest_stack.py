...
Runs load testing and seed operations using API service image on Fargate

Architecture:
- ECS Fargate task (on-demand, not always running)
- Uses ticketing-service ECR image (has Python, Go loadtest, seed scripts)
...
4. Run loadtest: Inside task, run Go loadtest binaries or Python seed scripts
    ...
    def __init__(
        ...
        **kwargs,
        ...
        loadtest_repo = ecr.Repository.from_repository_name(
            self, 'LoadTestRepo', repository_name='ticketing-loadtest'
        )
