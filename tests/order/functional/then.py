...
def verify_all_orders_status_paid(order_state):
    ...
    for order in orders:
        assert order.get('status') == 'paid', (
            f'Order {order["id"]} has status {order.get("status")}, expected paid'
        )
...
def verify_all_orders_status_pending(order_state):
    ...
    for order in orders:
        assert order.get('status') == 'pending_payment', (
            f'Order {order["id"]} has status {order.get("status")}, expected pending_payment'
        )
...
def verify_all_orders_status_cancelled(order_state):
    ...
    for order in orders:
        assert order.get('status') == 'cancelled', (
            f'Order {order["id"]} has status {order.get("status")}, expected cancelled'
        )
