...
def create_seller(step, client: TestClient, order_state):
    ...
    if response.status_code == 201:
        ...
        order_state['seller'] = {'id': 1, 'email': seller_data['email']}


@given('another buyer exists:')
def create_another_buyer(step, client: TestClient, order_state):
    ...
    if response.status_code == 201:
        buyer = response.json()
        order_state['another_buyer'] = buyer
    else:
        order_state['another_buyer'] = {'id': 3, 'email': buyer_data['email']}


@given('a product exists:')
def create_product_for_cancellation(step, client: TestClient, order_state, execute_sql_statement):
