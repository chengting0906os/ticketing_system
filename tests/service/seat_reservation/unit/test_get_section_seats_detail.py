"""
Tests for list_subsection_seats_detail functionality
...
from src.platform.state.kvrocks_client import KvrocksStatsClient
...
class TestGetSectionSeatsDetail:
    ...
    async def test_list_subsection_seats_detail_returns_500_seats(
        self, kvrocks_client, mock_kvrocks_client, monkeypatch
    ):
        """測試：應該返回 500 個座位資料"""
        # Arrange
        event_id = 1
        ...
        mock_kvrocks_client.exists = AsyncMock(return_value=1)
        ...
        mock_kvrocks_client.getrange = AsyncMock(return_value=bitfield_data)
        ...
        mock_kvrocks_client.pipeline = MagicMock(return_value=mock_pipeline)

        # Mock kvrocks_client.connect() - must be an AsyncMock that returns the client
        async_connect_mock = AsyncMock(return_value=mock_kvrocks_client)
        monkeypatch.setattr(
            'src.platform.state.kvrocks_client.kvrocks_client.connect', async_connect_mock
        )

        # Act
        result = await kvrocks_client.list_subsection_seats_detail(
    ...
    async def test_list_subsection_seats_detail_handles_partial_bitfield(
        self, kvrocks_client, mock_kvrocks_client, monkeypatch
    ):
    ...
    async def test_list_subsection_seats_detail_returns_empty_when_bitfield_not_found(
        self, kvrocks_client, mock_kvrocks_client, monkeypatch
    ):
        ...
        mock_kvrocks_client.exists = AsyncMock(return_value=0)
        ...
        mock_kvrocks_client.getrange.assert_not_called()
