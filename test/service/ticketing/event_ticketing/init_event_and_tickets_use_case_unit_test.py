...
class TestCreateEventKafkaSetup:
    ...
    @pytest.fixture
    def mock_mq_orchestrator(self) -> Mock:
    ...
    @pytest.fixture
    def use_case(
        self, mock_event_repo: Mock, mock_mq_orchestrator: Mock, mock_init_state_handler: Mock
    ) -> CreateEventAndTicketsUseCase:
    ...
    async def test_kafka_setup_called_after_event_creation(
        self,
        use_case: CreateEventAndTicketsUseCase,
        mock_event_repo: Mock,
        mock_mq_orchestrator: Mock,
        mock_init_state_handler: Mock,
        valid_event_data: dict[str, Any],
        created_aggregate: EventTicketingAggregate,
    ) -> None:
        """Test that Kafka topics/partitions are called after event creation"""
        # Given: Track call order
        call_order = []

        async def track_create_event(
            *, event_aggregate: EventTicketingAggregate
        ) -> EventTicketingAggregate:
            call_order.append('create_event')
            return created_aggregate

        async def track_batch_tickets(
            *,
            event_aggregate: EventTicketingAggregate,
            ticket_tuples: list[tuple[str, str, int, int]],
        ) -> EventTicketingAggregate:
            call_order.append('batch_tickets')
            return created_aggregate

        async def track_init_seats(
            *, event_id: int, seating_config: dict[str, Any]
        ) -> dict[str, Any]:
