...
def verify_user_details(step: Step, user_state: dict[str, Any]) -> None:
...
def verify_seller_user_details(step: Step, user_state: dict[str, Any]) -> None:
    expected = extract_table_data(step)
    request_data = user_state['request_data']
    assert request_data['email'] == expected['email']
    assert request_data['password'] == expected['password']
...
def verify_wrong_user_details(step: Step, user_state: dict[str, Any]) -> None:
    expected = extract_table_data(step)
    request_data = user_state['request_data']
    assert request_data['email'] == expected['email']
    assert request_data['password'] == expected['password']
...
def verify_login_success(user_state: dict[str, Any]) -> None:
...
def verify_login_failure(user_state: dict[str, Any]) -> None:
    response = user_state['response']
...
def verify_jwt_cookie(user_state: dict[str, Any]) -> None:
    response = user_state['response']
...
def verify_user_info(step: Step, user_state: dict[str, Any]) -> None:
    expected = extract_table_data(step)
    response = user_state['response']
