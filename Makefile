...
run:
	...
	set -a && source .env.example && set +a && uv run granian --interface asgi src.main:app --host 0.0.0.0 --port 8000 --reload --reload-path src --http auto --loop uvloop --log-config src/shared/logging/granian_log_config.json --access-log
