...
from src.platform.state.kvrocks_client import KvrocksClientType
...
class LuaScripts:
    ...
    async def initialize(self, *, client: KvrocksClientType) -> None:
        ...
        if find_seats_path.exists():
            ...
            # RedisCluster also supports register_script via ClusterCommands mixin
            self._find_consecutive_seats_script = client.register_script(  # type: ignore[union-attr]
        ...
        if verify_seats_path.exists():
            ...
            # RedisCluster also supports register_script via ClusterCommands mixin
            self._verify_manual_seats_script = client.register_script(  # type: ignore[union-attr]
    ...
    async def find_consecutive_seats(
        self, *, client: KvrocksClientType, keys: list[str], args: list[str]
    ) -> bytes | None:
        ...
        try:
            # RedisCluster supports Script execution via ClusterCommands
            return cast(
                bytes | None,
                await self._find_consecutive_seats_script(keys=keys, args=args, client=client),  # type: ignore[arg-type]
            )
    ...
    async def verify_manual_seats(
        ...
        try:
            ...
            return cast(
                bytes | None,
                await self._verify_manual_seats_script(keys=keys, args=args, client=client),  # type: ignore[arg-type]
            )
        except NoScriptError:
