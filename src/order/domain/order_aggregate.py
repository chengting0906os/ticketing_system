...
class OrderAggregate:
    ...
    def create_order(
        ...
        aggregate._add_event(OrderCreatedEvent(
    ...
    def process_payment(self) -> None:
        ...
        self._add_event(OrderPaidEvent(
    ...
    def cancel(self, reason: Optional[str] = None) -> None:
        ...
        self._add_event(OrderCancelledEvent(
            aggregate_id=self.order.id or 0,
            buyer_id=self.order.buyer_id,
            product_id=self.order.product_id,
            reason=reason
    ...
    def _reserve_product(self) -> None:
        if self._product:
            ...
            self._add_event(ProductReservedEvent(
                aggregate_id=self.order.id or 0,
    ...
    def _release_product(self) -> None:
        if self._product:
            ...
            self._add_event(ProductReleasedEvent(
                aggregate_id=self.order.id or 0,
                product_id=self._product.id or 0,
                order_id=self.order.id or 0
            ))
