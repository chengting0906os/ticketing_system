...
Verifies specified seats are available.
Returns validated seat data for atomic reservation.

KEYS[1]: Bitfield key (e.g., 'seats_bf:1:A-1')

ARGV[1]: cols (seats per row, for calculating seat_index)
ARGV[2+]: seat_ids in "row-seat" format (e.g., '1-5', '1-6', '1-7')
...
-- Collect seat_ids from ARGV[2] onwards
local seat_ids = {}
for i = 2, #ARGV do
