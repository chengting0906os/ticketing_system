...
class UpdateBookingToPaidUseCase:
    ...
    def depends(
        ...
        booking_repo: BookingCommandRepo = Depends(get_booking_command_repo),
    ):
        return cls(session, booking_repo)

    @Logger.io
    async def update_to_paid(self, booking: Booking) -> Booking:
        paid_booking = booking.mark_as_paid()
        updated_booking = await self.booking_repo.update_status_to_paid(booking=paid_booking)
